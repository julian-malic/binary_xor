...
int main(int argc, char* argv[]) {
	...
	for (auto c = 0; c < buffer.size(); c++) { buffer[c] ^= key; } // xor each byte with key
	for (auto obj : buffer) { output_file.write((const char*)&obj, sizeof(obj)); } // write file with encrypted bytes
